...
public class FavoriteService {
    ...
    public void setFavoriteItem(UserEntity user, ItemEntity item) {
        ...
        if (persistedItem == null) {//看看(我们的SQL，不是Twitch的DB)DB里有没有
            persistedItem = itemRepository.save(item);
        }
        if (favoriteRecordRepository.existsByUserIdAndItemId(user.id(), persistedItem.id())) {//已经favorite过了
            throw new DuplicateFavoriteException();//extends runtime exception来区分
        }
        FavoriteRecordEntity favoriteRecord = new FavoriteRecordEntity(null, user.id(), persistedItem.id(), Instant.now());//favorite
